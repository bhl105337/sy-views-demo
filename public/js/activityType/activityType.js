$(function () {
    ...
    $.ajax({
        url: 'http://192.168.2.199:3000/admin/getClsActive',
        type: 'GET',
        cache: false,
        data: {start: 0},
        processData: false,
    ...
    $.ajax({
        url: 'http://192.168.2.199:3000/admin/gameName',
        type: 'GET',
        cache: false,
        data: {start: 0},
        processData: false,
    ...
    $('#activity_table').on('click','.delete_activity',function () {
        ...
        Ewin.confirm({ message: "确认要删除吗？" }).on(function (e) {
            if (!e) {
                ...
                console.log(name,id);
                $.ajax({
                    url: 'http://192.168.2.199:3000/admin/deleteActive',
                    type: 'GET',
    ...
    $('#add_activity_btn').click(function () {
        ...
        // formData.append('arr',GameIdArr);
        // formData.append('type',checkArr);
        // GameIdArr.join(";");
        // var GameIdArr=JSON.stringify(GameIdArr);
        // var checkArr=JSON.stringify(checkArr);

            // $.ajax({
            //     url: 'http://192.168.2.199:3000/admin/setClsActive',
            //     type: 'Get',
            //     cache: false,
            //     data: '{"type": type_game, "arr": GameIdArr}',
            //     processData: false,
            //     contentType: false,
            //     success: function (data) {
    ...
    function  Getajax(type_game , ID) {
        ...
        $.ajax({
            ...
            type: 'get',
            cache: false,
            data: {type: "alone", arr: ["30", "41", "42", "43"] },
            processData: false,
            contentType: false,
            success: function (data) {
                console.log(data.state);
    ...
    $('#gameType').click(function () {
       ...
       var gameTypeUrl = ["alone","online","application"];
        for (var i =0;i < gameTypeUrl.length; ++i){
           $.ajax({
               url: 'http://192.168.2.199:3000/admin/gameByType?msg='+ gameTypeUrl[i],
               type: 'GET',
               async: false,
               cache: false,
               data: {"msg": "typeGame"},
